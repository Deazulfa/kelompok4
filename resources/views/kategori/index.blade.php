...
<style>
    ...
    <div class="container mt-5">
        ...
        <a href="{{ route('category.create') }}" class="btn btn-dark">+ Tambah Kategori</a>
                ...
                @foreach ($category as $item)
                    <tr>
                        ...
                        <td class="d-flex justify-content-beetwen">
                            <a href="{{ route('category.edit', $item->id) }}" class="btn btn-warning me-2 text-light">
                                Edit
                            </a>
                            ...
                            <form action="{{ route('category.destroy', $item->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger me-2" onclick="return confirm('Are you sure you want to delete this category?')">Delete</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
